...
#### 版本 1.0.5 - 2026-01-26 - 功能优化和bug修复
- 修复Tab键聚焦顺序问题，实现搜索框→盘符列表→文件列表的循环聚焦
- 修复文件列表聚焦时边框变色提醒
- 修复盘符列表上下选择功能
- 修复Backspace键返回上一级目录功能
- 优化滚动条样式，最终实现滚动条隐藏
- 修复isKeyPressed函数逻辑错误
- 修复浏览器环境中使用require函数的错误

#### 版本 1.1.0 - 2026-01-26 - 路径栏精准回退功能
- 实现路径栏精准回退功能，将路径中的每一级作为跳转按钮
- 点击任意级目录即可直接跳转到该目录
- 支持Windows和Unix两种路径格式
- 为路径段添加了hover和active效果
- 优化了长路径的显示效果，支持自动换行

#### 版本 1.1.1 - 2026-01-26 - 文件栏聚焦与滚动同步修复
- 修复文件栏聚焦与滚动不同步问题
- 将滚动行为从smooth改为auto，消除滚动延迟
- 优化键盘导航体验，确保聚焦项立即可见
- 同时修复了盘符列表和文件列表的滚动问题

#### 版本 1.1.2 - 2026-01-26 - 复制粘贴剪切快捷键实现
- 实现文件复制、剪切、粘贴快捷键操作
- 添加剪贴板状态管理，记住用户复制或剪切的内容
- 在preload.js中添加文件操作API（copyFiles, moveFiles）
- 支持Ctrl+C复制、Ctrl+X剪切、Ctrl+V粘贴快捷键
- 实现多选支持，包括Ctrl+点击切换选中和Shift+点击连续选择
- 优化文件选择交互，添加选中状态管理
- 实现操作结果反馈，显示成功或失败消息

#### 版本 1.1.3 - 2026-01-26 - 新建文件/文件夹快捷键实现
- 实现Ctrl+N快捷键新建文件/文件夹功能
- 添加Windows命名规则检测，确保文件名合法性
- 创建新建文件/文件夹弹窗UI，支持输入包含扩展名的名称
- 实现智能判断：包含扩展名创建文件，否则创建文件夹
- 在preload.js中添加createFile和createDirectory API
- 添加弹窗交互逻辑，支持Enter确认、Escape取消
- 实现操作结果反馈，显示成功或失败消息

#### 版本 1.1.4 - 2026-01-26 - 新建文件窗口交互优化
- 将新建文件窗口的取消按键从Escape改为Backspace
- 优化Backspace取消逻辑：只有在输入框为空时才取消
- 修改按键提示样式：移除背景框，改为绿色文字
- 更新按键提示文本：将"Escape: 取消"改为"Backspace: 取消"
...
| 1.0.4 | 2026-01-23 | 键盘导航功能 | 已完成 |
| 1.0.5 | 2026-01-26 | 功能优化和bug修复 | 已完成 |
| 1.1.0 | 2026-01-26 | 路径栏精准回退功能 | 已完成 |
| 1.1.1 | 2026-01-26 | 文件栏聚焦与滚动同步修复 | 已完成 |
| 1.1.2 | 2026-01-26 | 复制粘贴剪切快捷键实现 | 已完成 |
| 1.1.3 | 2026-01-26 | 新建文件/文件夹快捷键实现 | 已完成 |
| 1.1.4 | 2026-01-26 | 新建文件窗口交互优化 | 当前版本 |
| 1.1.5 | 待定 | 根据后续需求开发 | 规划中 |
